...
- 优先查看 `result.json.stepResults[].details.networkEvidence`，再决定是否展开更深的网络证据
- 可以先运行 `node skills/miniprogram-minium-cli/scripts/filter-network-artifact.mjs --result /path/to/result.json --pretty`，把 `network.json` 收缩成和当前 step 相关的低噪音子图
- 当你需要完整证据时，再查看完整 `network.json`，其中包含 run 级归一化事件日志以及 `requests`、`listeners`、`intercepts` 索引
...
- `network.json` 的固定结构为 `schemaVersion`、`events`、`requests`、`listeners`、`intercepts`、`meta`
- `result.json.stepResults[]` 可能包含 `details.networkEvidence`，调用方可以据此稳定回跳到 `network.json`，而不必再从各类 step 输出字段里自行猜测
