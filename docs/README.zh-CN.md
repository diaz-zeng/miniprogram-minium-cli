...
当包已经在本地可用时，可以通过 `pnpm exec` 安装随包 skills：

```bash
pnpm exec miniprogram-minium-cli install --skills
```

如果还没有全局安装，也可以直接通过 `pnpm dlx` 安装：
...
如果对应 agent 已经支持开放的 `skills` 生态，也可以直接执行 `pnpm dlx skills add diaz-zeng/miniprogram-minium-cli --skill miniprogram-minium-cli` 从仓库安装。
