...
### 决策三：能力按“监听/等待/断言/拦截”四类 step 暴露，而不是做一个大而全的万能 step
...
### 决策四：真实运行时优先基于受控网络 hook 做实现，placeholder 运行时复用同一契约
...
### 决策五：把网络证据做成独立运行产物，同时在步骤输出里保留摘要
